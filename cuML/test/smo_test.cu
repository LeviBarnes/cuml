...
void linearKernel(const float* workset, int ws_rows, int ws_cols, 
                  const float *tile, float *output, int out_rows, int out_cols,
                  cublasOperation_t trans_ws, cublasOperation_t trans_tile,
                  float alpha, float beta, cublasHandle_t h)
{
    LinAlg::gemm(workset, ws_rows, ws_cols, tile, output, out_rows, out_cols,
                 trans_ws, trans_tile, alpha, beta, h);
...
TEST(SmoSolverTest, KernelCacheTest) {
    ...
    KernelCache<float> *cache = new KernelCache<float>(x_dev, n_rows, n_cols, n_ws, cublas_handle, LinAlg::gemm);
